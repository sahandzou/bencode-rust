...
fn parse(i: &[u8]) -> IResult<&[u8], BValue> {
    ...
    let b_string = map(parse_string, BValue::BString);
...
mod tests {
    ...
    fn test_parse_string() {
        assert_eq!(parse(b"4:spam"), Ok((&b""[..], BValue::BString("spam".into()))));
    }

    #[test]
    fn test_parse_list() {
        assert_eq!(parse(b"l4:spami42ee"), Ok((&b""[..], BValue::BList(vec![BValue::BString("spam".into()), BValue::BNumber(42)]))));
    }

    #[test]
    fn test_parse_dict() {
        let dict = HashMap::from([
            ("bar".to_string(), BValue::BString("spam".into())),
